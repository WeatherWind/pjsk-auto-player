...
1. **ML 轻量 note 检测** → 用 ONNX 部署轻量模型 (如 YOLO-nano / MobileNet) 替换 OpenCV 形态学检测, 提高 flick 和 hold 识别率, 预计检测速度提升 3x
2. **WebSocket 实时推流** → 在 Web 仪表盘实时显示手机画面 (替代 opencv 窗口)
3. **minitouch 预编译二进制** → 内置主流架构 (arm64/arm/x86_64) minitouch, 开箱即用无需下载
4. **谱面缓存** → 热门歌曲缓存轨道位置和速度参数, 减少重复校准

### 中期 (v4.8.x — v5.0.x)

5. **自适应延迟 PID 控制器** → 基于实时 AP/FC 反馈自动调整延迟补偿, 替代固定补偿值
6. **多设备并行** → 一台电脑同时控制多台手机冲榜, 共享分析引擎
7. **谱面解析** → 直接解析 PJSK 谱面文件格式, 实现完美时序触发, 不再依赖视觉检测
8. **Native GUI (PyQt/Wails)** → 原生的桌面应用, 替代 Web 仪表盘, 更低开销

### 长期 (v5.x+)

9. **机器学习谱面适配** → 根据每首歌的 note 密度、速度变化自动选择最佳检测参数
10. **分布式冲榜集群** → 多台电脑/手机通过中央调度器协作冲榜
11. **反检测机制升级** → 更精细的人类行为模型: 疲劳曲线、注意力波动、反应速度变化

## 免责声明 & 法律合规

**⚠️ 重要法律提示**

本项目仅供学习和研究自动化技术原理使用。根据 SEGA / Colorful Palette 发布的《プロジェクトセカイ カラフルステージ！ feat. 初音ミク利用規約》**第9条**，以下行为被明确禁止：

> 「自動操作ツール（マクロ）の利用及び弊社が認めないプログラム・マクロその他のツールを使用し本サービスを利用すること」
> — 即：使用自动操作工具（宏/脚本）进行游戏

PJSK Auto Player 属于上述条款定义的"自动操作工具"。**使用本软件可能违反 Project Sekai 的服务条款，并可能导致账号封禁。**

### 风险降低建议

| 措施 | 说明 | 默认启用 |
|------|------|---------|
| **点击随机化** | 时机抖动 ±15ms、坐标偏移 ±5px、随机漏键 0.1% | ✅ |
| **模式浮动** | 冲榜时随机切换 AP/FC/LIVE，模拟人类发挥波动 | ✅ |
| **限制运行时长** | 避免 24 小时不间断运行 | ❌ (需手动控制) |
| **不要追求极端 AP** | 每首歌都 AP 是典型机器人特征 | 建议 FC 模式 |
| **不公开讨论** | 不要在游戏内聊天或官方社区提及本工具 | — |

### 完整协议

详细的用户协议和法律条款请参阅 [TERMS.md](TERMS.md)。

### 关系声明

本项目与以下公司没有任何关联，也未获得其认可或赞助：
- **SEGA Corporation**
